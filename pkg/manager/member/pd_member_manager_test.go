...
func TestPDMemberManagerSyncCreate(t *testing.T) {
	...
	type testcase struct {
		name                       string
		prepare                    func(cluster *v1alpha1.TikvCluster)
...
func TestPDMemberManagerSyncUpdate(t *testing.T) {
	...
	type testcase struct {
		name                       string
		modify                     func(cluster *v1alpha1.TikvCluster)
		...
		expectTikvClusterFn        func(*GomegaWithT, *v1alpha1.TikvCluster)
	}

	tests := []testcase{
		{
			name: "normal",
			modify: func(tc *v1alpha1.TikvCluster) {
			...
			expectTikvClusterFn: func(g *GomegaWithT, tc *v1alpha1.TikvCluster) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.errWhenGetPDHealth {
				...
				pdClient.AddReaction(pdapi.GetHealthActionType, func(action *pdapi.Action) (interface{}, error) {
					return tt.pdHealth, nil
				})
			}

			if tt.errWhenGetCluster {
			...
			if tt.statusChange == nil {
				...
				fakeSetControl.SetStatusChange(tt.statusChange)
			}
			...
			if tt.expectPDServiceFn != nil {
				svc, err := pmm.svcLister.Services(ns).Get(controller.PDMemberName(tcName))
				tt.expectPDServiceFn(g, svc, err)
			}
			if tt.expectPDPeerServiceFn != nil {
				svc, err := pmm.svcLister.Services(ns).Get(controller.PDPeerMemberName(tcName))
				tt.expectPDPeerServiceFn(g, svc, err)
			}
			if tt.expectStatefulSetFn != nil {
				set, err := pmm.setLister.StatefulSets(ns).Get(controller.PDMemberName(tcName))
				tt.expectStatefulSetFn(g, set, err)
			}
			if tt.expectTikvClusterFn != nil {
				tt.expectTikvClusterFn(g, tc1)
			}
		})
	}
}
...
func TestPDMemberManagerUpgrade(t *testing.T) {
	...
	testFn := func(test *testcase, t *testing.T) {
		...
		if test.expectTikvClusterFn != nil {
			test.expectTikvClusterFn(g, tc1)
		}
	}
	tests := []testcase{
		{
...
func TestPDMemberManagerSyncPDSts(t *testing.T) {
	...
	tests := []testcase{
		{
			...
			modify: func(cluster *v1alpha1.TikvCluster) {
				...
				cluster.ObjectMeta.Annotations["tikv.org/force-upgrade"] = "true"
			},
			pdHealth: &pdapi.HealthInfo{Healths: []pdapi.MemberHealth{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			err = pmm.syncPDStatefulSetForTikvCluster(tc)
			if tt.err {
				g.Expect(err).To(HaveOccurred())
			} else {
				g.Expect(err).NotTo(HaveOccurred())
...
func TestGetNewPDHeadlessServiceForTikvCluster(t *testing.T) {
	tests := []struct {
		name     string
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			svc := getNewPDHeadlessServiceForTikvCluster(&tt.tc)
...
func TestGetNewPDSetForTikvCluster(t *testing.T) {
	enable := true
	tests := []struct {
		name    string
		tc      v1alpha1.TikvCluster
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			sts, err := getNewPDSetForTikvCluster(&tt.tc, nil)
...
func TestGetNewPdServiceForTikvCluster(t *testing.T) {
	tests := []struct {
		name     string
		tc       v1alpha1.TikvCluster
		expected corev1.Service
...
func TestPDMemberManagerSyncPDStsWhenPdNotJoinCluster(t *testing.T) {
	...
	type testcase struct {
		name                string
		modify              func(cluster *v1alpha1.TikvCluster, podIndexer cache.Indexer, pvcIndexer cache.Indexer)
		pdHealth            *pdapi.HealthInfo
		tcStatusChange      func(cluster *v1alpha1.TikvCluster)
		err                 bool
		expectTikvClusterFn func(*GomegaWithT, *v1alpha1.TikvCluster)
	}
