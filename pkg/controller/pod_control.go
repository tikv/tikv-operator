...
func (rpc *realPodControl) UpdatePod(tc *v1alpha1.TikvCluster, pod *corev1.Pod) (*corev1.Pod, error) {
	ns := tc.GetNamespace()
	tcName := tc.GetName()
	podName := pod.GetName()
	...
	err := retry.RetryOnConflict(retry.DefaultRetry, func() error {
		...
		if updateErr == nil {
			klog.Infof("Pod: [%s/%s] updated successfully, TikvCluster: [%s/%s]", ns, podName, ns, tcName)
			return nil
		}
...
func (rpc *realPodControl) UpdateMetaInfo(tc *v1alpha1.TikvCluster, pod *corev1.Pod) (*corev1.Pod, error) {
	ns := pod.GetNamespace()
	podName := pod.GetName()
	labels := pod.GetLabels()
	tcName := tc.GetName()
	if labels == nil {
		return pod, fmt.Errorf("pod %s/%s has empty labels, TikvCluster: %s", ns, podName, tcName)
	}
	...
	if labels[label.ClusterIDLabelKey] == "" {
		...
		if err != nil {
			return pod, fmt.Errorf("failed to get tikv cluster info from pd, TikvCluster: %s/%s, err: %v", ns, tcName, err)
		}
	...
	case label.PDLabelVal:
		if labels[label.MemberIDLabelKey] == "" {
			...
			if err != nil {
				return pod, fmt.Errorf("failed to get pd members info from pd, TikvCluster: %s/%s, err: %v", ns, tcName, err)
			}
	...
	case label.TiKVLabelVal:
		if labels[label.StoreIDLabelKey] == "" {
			...
			if err != nil {
				return pod, fmt.Errorf("failed to get tikv stores info from pd, TikvCluster: %s/%s, err: %v", ns, tcName, err)
			}
	...
	if labels[label.ClusterIDLabelKey] == clusterID &&
		...
		klog.V(4).Infof("pod %s/%s already has cluster labels set, skipping. TikvCluster: %s", ns, podName, tcName)
		return pod, nil
	}
	...
	err := retry.RetryOnConflict(retry.DefaultBackoff, func() error {
		...
		updatePod, updateErr = rpc.kubeCli.CoreV1().Pods(ns).Update(pod)
		if updateErr == nil {
			klog.V(4).Infof("update pod %s/%s with cluster labels %v successfully, TikvCluster: %s", ns, podName, labels, tcName)
			return nil
		}
		klog.Errorf("failed to update pod %s/%s with cluster labels %v, TikvCluster: %s, err: %v", ns, podName, labels, tcName, updateErr)
...
func (rpc *realPodControl) DeletePod(tc *v1alpha1.TikvCluster, pod *corev1.Pod) error {
	ns := tc.GetNamespace()
	tcName := tc.GetName()
	podName := pod.GetName()
	...
	if err != nil {
		klog.Errorf("failed to delete Pod: [%s/%s], TikvCluster: %s, %v", ns, podName, tcName, err)
	} else {
		klog.V(4).Infof("delete Pod: [%s/%s] successfully, TikvCluster: %s", ns, podName, tcName)
	}
...
func (rpc *realPodControl) recordPodEvent(verb string, tc *v1alpha1.TikvCluster, podName string, err error) {
	...
	if err == nil {
		...
		msg := fmt.Sprintf("%s Pod %s in TikvCluster %s successful",
			strings.ToLower(verb), podName, tcName)
		rpc.recorder.Event(tc, corev1.EventTypeNormal, reason, msg)
	} else {
		reason := fmt.Sprintf("Failed%s", strings.Title(verb))
		msg := fmt.Sprintf("%s Pod %s in TikvCluster %s failed error: %s",
