...
type PVCControlInterface interface {
	UpdateMetaInfo(*v1alpha1.TikvCluster, *corev1.PersistentVolumeClaim, *corev1.Pod) (*corev1.PersistentVolumeClaim, error)
	UpdatePVC(*v1alpha1.TikvCluster, *corev1.PersistentVolumeClaim) (*corev1.PersistentVolumeClaim, error)
	DeletePVC(*v1alpha1.TikvCluster, *corev1.PersistentVolumeClaim) error
...
func (rpc *realPVCControl) DeletePVC(tc *v1alpha1.TikvCluster, pvc *corev1.PersistentVolumeClaim) error {
	ns := tc.GetNamespace()
	tcName := tc.GetName()
	pvcName := pvc.GetName()
	err := rpc.kubeCli.CoreV1().PersistentVolumeClaims(tc.GetNamespace()).Delete(pvcName, nil)
	if err != nil {
		klog.Errorf("failed to delete PVC: [%s/%s], TikvCluster: %s, %v", ns, pvcName, tcName, err)
	}
	klog.V(4).Infof("delete PVC: [%s/%s] successfully, TikvCluster: %s", ns, pvcName, tcName)
	rpc.recordPVCEvent("delete", tc, pvcName, err)
	return err
}

func (rpc *realPVCControl) UpdatePVC(tc *v1alpha1.TikvCluster, pvc *corev1.PersistentVolumeClaim) (*corev1.PersistentVolumeClaim, error) {
	ns := tc.GetNamespace()
	tcName := tc.GetName()
	pvcName := pvc.GetName()
	...
	err := retry.RetryOnConflict(retry.DefaultBackoff, func() error {
		...
		if updateErr == nil {
			klog.Infof("update PVC: [%s/%s] successfully, TikvCluster: %s", ns, pvcName, tcName)
			return nil
		}
		klog.Errorf("failed to update PVC: [%s/%s], TikvCluster: %s, error: %v", ns, pvcName, tcName, updateErr)
...
func (rpc *realPVCControl) UpdateMetaInfo(tc *v1alpha1.TikvCluster, pvc *corev1.PersistentVolumeClaim, pod *corev1.Pod) (*corev1.PersistentVolumeClaim, error) {
	ns := tc.GetNamespace()
	tcName := tc.GetName()
	pvcName := pvc.GetName()
	podName := pod.GetName()
	...
	if pvc.Labels[label.ClusterIDLabelKey] == clusterID &&
		...
		klog.V(4).Infof("pvc %s/%s already has labels and annotations synced, skipping, TikvCluster: %s", ns, pvcName, tcName)
		return pvc, nil
	}
	...
	err := retry.RetryOnConflict(retry.DefaultBackoff, func() error {
		...
		updatePVC, updateErr = rpc.kubeCli.CoreV1().PersistentVolumeClaims(ns).Update(pvc)
		if updateErr == nil {
			klog.V(4).Infof("update PVC: [%s/%s] successfully, TikvCluster: %s", ns, pvcName, tcName)
			return nil
		}
		klog.Errorf("failed to update PVC: [%s/%s], TikvCluster: %s, error: %v", ns, pvcName, tcName, updateErr)
...
func (rpc *realPVCControl) recordPVCEvent(verb string, tc *v1alpha1.TikvCluster, pvcName string, err error) {
	tcName := tc.GetName()
	if err == nil {
		reason := fmt.Sprintf("Successful%s", strings.Title(verb))
		msg := fmt.Sprintf("%s PVC %s in TikvCluster %s successful",
			strings.ToLower(verb), pvcName, tcName)
		rpc.recorder.Event(tc, corev1.EventTypeNormal, reason, msg)
	} else {
		reason := fmt.Sprintf("Failed%s", strings.Title(verb))
		msg := fmt.Sprintf("%s PVC %s in TikvCluster %s failed error: %s",
